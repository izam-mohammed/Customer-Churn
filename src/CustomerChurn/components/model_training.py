...
class ModelTrainer:
    ...
    def _randomized_search(self, name, clf, params, runs=50):
        ...
        rand_clf = RandomizedSearchCV(
            clf, params, n_iter=runs, cv=5, n_jobs=4, random_state=2, verbose=1
        )
    ...
    def train(self):
                ...
                "GradientBoost": {
                    ...
                    "auto": {
                        ...
                        "min_samples_split": uniform(
                            0.1, 1.0
                        ),  # Uniform distribution for min_samples_split
                        "min_samples_leaf": uniform(
                            0.1, 0.5
                        ),  # Uniform distribution for min_samples_leaf
                        "subsample": uniform(
                            0.5, 1.0
                        ),  # Uniform distribution for subsample
                        "max_features": ["auto", "sqrt", "log2", None],
                    },
                },
                "AdaBoost": {
        ...
        for model in models:
            ...
            if model == "XGBoost":
                clf_model, score = self._randomized_search(
                    name=str(model), clf=clf, params=params, runs=300
                )
            else:
                clf_model, score = self._randomized_search(
        ...
        save_bin_dup(
            data=best_model,
            path=Path(os.path.join(self.config.permanent_path, "model.joblib")),
        )
